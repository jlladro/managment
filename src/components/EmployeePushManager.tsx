...
export default function EmployeePushManager() {
  ...
  const requestPermission = async () => {
    if (!employeeName) return; // Nicht angemeldet
    setDbError(null);
    ...
    try {
      ...
      if (result === 'granted') {
        ...
        // Finde den User anhand des Namens
        const user = demoDb.db.users.find(u => u.name === employeeName);
        if (!user) throw new Error("Mitarbeiter nicht in DB gefunden");
    ...
    <div className="fixed bottom-4 right-4 z-[60] animate-in slide-in-from-bottom-10">
      <div className="bg-[#12161F]/90 border border-orange-500/30 p-6 rounded-[32px] shadow-2xl flex flex-col items-center text-center max-w-[280px] backdrop-blur-md">
        ...
        <p className="text-slate-500 text-xs mb-5 leading-relaxed">
          Benachrichtigungen für Baustellen und Materialien aktivieren?
        </p>
